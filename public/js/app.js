/* =========================
   Inventory SPA — single-file JS
   (Vanilla JS, Firebase RTDB compat v9)
   ========================= */
...
if (!window.firebase || !firebase.initializeApp) {
  console.error('Firebase SDK missing. Ensure compat v9 scripts are loaded in index.html');
}
...
/* Auth persistence without eval / IDB guard */
...
const cloud = (function(){
  ...
  async function saveKV(key,val){
    if(!on() || !uid()) return;
    const row={ key, val, updatedAt: firebase.database.ServerValue.TIMESTAMP };
    await pathFor(key).set(row);
  }
  ...
  function unsubscribeAll(){
    if(!uid()) return;
    CLOUD_KEYS.forEach(key=>{ try{ pathFor(key).off(); }catch{} });
  }
  ...
  async function enable(){ if(!uid()) throw new Error('Sign in first'); set(true); try{ await firebase.database().goOnline(); }catch{} await pullAllOnce(); await pushAll(); subscribeAll(); }
  async function disable(){ set(false); unsubscribeAll(); try{ await firebase.database().goOffline(); }catch{} }
...
/* ---------- Theme (soft palettes + immediate size changes) ---------- */
const THEME_MODES = [
...
if (auth && typeof auth.onAuthStateChanged === 'function'){
  ...
  // fallback local mode
  (function(){
    const s = safeJSON(localStorage.getItem('session'), null);
    if (s && s.authMode==='local'){ state.session = s; seedTenantOnce(); renderApp(); }
    else renderLogin();
  })();
}
...
async function ensureSessionAndRender(user){
  ...
  // Firebase user -> local users list (for role + registry)
...
function renderSidebar(active='dashboard'){
  ...
  return `
    <aside class="sidebar" id="sidebar">
      ...
      <!-- Mobile-first search in sidebar -->
...
document.addEventListener('click', (e)=>{
  ...
  // Mobile: collapse drawer and bring main into view
...
function wireRoute(route){
  ...
  switch(route){
    ...
    case 'search':    /* nothing extra */ break;
  }
}
...
// COGS (no Delivery column; right-aligned; G-Income & G-Profit)
function viewCOGS(){
...
// Tasks (DnD with empty-lane acceptance + highlight)
function viewTasks(){
...
// Settings (Theme + Cloud + Users with roles)
  ...
  toggle?.addEventListener('change', async (e)=>{
    ...
    try{
      if (val==='on'){
        ...
        await firebase.database().goOnline(); await cloud.enable(); state.cloudOn=true; notify('Cloud Sync ON');
      }else{
